...
describe("databasesDynamoDBSingleTable", () => {
  ...
  it("should use generic names for LSI", () => {
    for (let localSecondaryIndex = 1; localSecondaryIndex <= 5; localSecondaryIndex++) {
      expect(
        cfTemplate.Resources[computeLogicalId("databaseWithLocalSecondaryIndexes", "Table")].Properties.AttributeDefinitions
      ).toContainEqual({ AttributeName: `LSI-${localSecondaryIndex}-SK`, AttributeType: "S" });
    }
    expect(
      cfTemplate.Resources[computeLogicalId("databaseWithLocalSecondaryIndexes", "Table")].Properties.LocalSecondaryIndexes
    ).toEqual(
  ...
  it("should use generic names for GSI", () => {
    for (let globalSecondaryIndex = 1; globalSecondaryIndex <= 2; globalSecondaryIndex++) {
      expect(
        cfTemplate.Resources[computeLogicalId("databaseWithGlobalSecondaryIndexes", "Table")].Properties.AttributeDefinitions
    ...
    expect(
      cfTemplate.Resources[computeLogicalId("databaseWithGlobalSecondaryIndexes", "Table")].Properties.GlobalSecondaryIndexes
    ).toEqual(
